...
import { ILinkedComponent, isLinkedComponent } from "./linked-component";
...
export class Entity implements ReadonlyEntity {
   ...
   */
  public readonly onComponentAdded: Signal<ComponentUpdateHandler> =
    new Signal();
  /**
   * The signal dispatches if component was removed from the entity. Works for every linked component as well.
   */
  public readonly onComponentRemoved: Signal<ComponentUpdateHandler> =
    new Signal();
  /**
   ...
   */
  public readonly onInvalidationRequested: Signal<(entity: Entity) => void> =
    new Signal();
  ...
  private _linkedComponents: Record<
    number,
    LinkedComponentList<ILinkedComponent>
  > = {};
   ...
   * ```
   */
  public add<T extends K, K extends unknown>(
   ...
   * ```
   */
  public append<T extends K, K extends ILinkedComponent>(
  ...
  public withdraw<T>(componentClass: Class<T>): T | undefined {
    ...
    if (isLinkedComponent(component)) {
      return this.withdrawComponent(
        component,
        componentClass as Class<ILinkedComponent>,
      );
    } else {
   ...
   */
  public pick<T extends ILinkedComponent>(
  ...
  public pick<T>(
    ...
    if (typeof resolveClassOrId === "string") {
      ...
      if (isLinkedComponent(component)) {
        return this.withdrawComponent(
          component,
          componentOrResolveClass as Class<ILinkedComponent>,
        );
      }
      return undefined;
    }
    if (isLinkedComponent(componentOrResolveClass)) {
      return this.withdrawComponent(
        ...
        resolveClassOrId as Class<ILinkedComponent>,
      );
    }
    return this.remove(
      resolveClassOrId ??
   ...
   * ```
   */
  public addComponent<T extends K, K extends unknown>(
    component: NonNullable<T>,
    resolveClass?: Class<K>,
  ): Entity {
    ...
    if (linkedComponent) {
      this.append(
        component as ILinkedComponent,
        resolveClass as Class<ILinkedComponent>,
      );
    } else {
   ...
   * ```
   */
  public appendComponent<T extends K, K extends ILinkedComponent>(
    component: NonNullable<T>,
    resolveClass?: Class<K>,
  ): Entity {
    const componentClass = getComponentClass(component, resolveClass);
  ...
  public find<T>(
    ...
    return predicate(component as T) ? (component as T) : undefined;
  }
  ...
  public takeSnapshot<T>(
    ...
    if (isTag(changedComponentOrTag)) {
      ...
      const componentClass =
        resolveClass ??
        Object.getPrototypeOf(changedComponentOrTag).constructor;
      const componentId = getComponentId(componentClass!, true)!;
  ...
  public getLinkedComponentList(
    ...
    if (
      ...
      return (this._linkedComponents[componentClassOrId] =
        new LinkedComponentList<ILinkedComponent>());
    }
  }

  private withdrawComponent<T extends K, K extends ILinkedComponent>(
    component: NonNullable<T>,
    resolveClass?: Class<K>,
  ): T | undefined {
    const componentClass = getComponentClass(component, resolveClass);
