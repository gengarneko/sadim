...
export class Engine {
  ...
  public addSystem(system: System, priority: number = 0): Engine {
    ...
    if (this._systems.length === 0) {
      ...
      const index = this._systems.findIndex(
  ...
  private connectEntity(entity: Entity) {
    ...
    entity.onInvalidationRequested.connect(
  ...
  private onComponentAdded = <T>(
    entity: Entity,
    component: NonNullable<T>,
    componentClass?: Class<NonNullable<T>>,
  ) => {
    this._queries.forEach((value) =>
      value.entityComponentAdded(entity, component, componentClass),
    );
  };
  ...
  private onComponentRemoved = <T>(
    entity: Entity,
    component: NonNullable<T>,
    componentClass?: Class<NonNullable<T>>,
  ) => {
    this._queries.forEach((value) =>
      value.entityComponentRemoved(entity, component, componentClass),
    );
  };
}
...
export class Subscription<T> {
  ...
  public equals(messageType: Class<T> | T, handler?: (message: T) => void) {
    return (
      ...
      (handler === undefined || this.handler === handler)
    );
  }
}
