...
export const EntityComponent: React.ForwardRefRenderFunction<EntityRef, EntityProps> = (
  { children, entity: providedEntity },
  ref
) => {
  const { engine } = React.useContext(ECSContext);
  const entity = React.useMemo(() => providedEntity || new _Entity(), [providedEntity]);
